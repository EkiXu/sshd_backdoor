...
#ifdef DEBUG
bpf_printk("target buffer_addr:%x",data.buffer_addr);

char head[17] = {0};

//bpf_probe_read_user((char *)buff_addr, 16,head);

//bpf_printk("target header:%s",head);
...
#ifdef DEBUG
bpf_printk("get target buffer_addr:%x",data->buffer_addr);
#endif
...
#ifdef DEBUG
    bpf_printk("custom playload:%s len:%d",payload->raw_buf,payload->payload_len);
#endif
    ...
    */
    ...
    long ret = bpf_probe_write_user((void *)new_buff_addr, payload->raw_buf, MAX_PAYLOAD_LEN); // Works
    //long ret = bpf_probe_write_user((void *)new_buff_addr, payload->raw_buf, payload->payload_len); //disallow
